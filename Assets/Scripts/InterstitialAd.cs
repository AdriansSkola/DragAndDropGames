...
public class InterstitialAd : MonoBehaviour, IUnityAdsLoadListener, IUnityAdsShowListener
{
    ...
    {
        Debug.Log("Interstitial ad clicked by user!");
    }
    ...
    public void OnUnityAdsShowComplete(string placementId, UnityAdsShowCompletionState showCompletionState)
    {
        ...
        {
            Debug.Log("Interstitial ad was skipped before reaching the end.");
            LoadAd();
        }
    }
    ...
    private IEnumerator SlowDownTimeTemporarily(float seconds)
    {
        ...
        Time.timeScale = 0.4f;
        Debug.Log("Time slowed down to 0.4x "+ seconds +" sec");
        yield return new WaitForSecondsRealtime(seconds);
        Time.timeScale = 1.0f;
        Debug.Log("Time restored to normal"); // Restore original time scale
    }
